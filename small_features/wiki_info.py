...
def wiki_search(query, lang='en'):
    ...
    try:
        ...
        return f"Sorry, I couldn't find anything about <{query}>\n" \
               f"Please try it yourself: https://en.wikipedia.org/wiki/{query}"
    except wiki.DisambiguationError:
        return f"Sorry, your query is too vague, make it more clear and ask again!\n" \
               f"Or you can just try to find it yourself: https://en.wikipedia.org/wiki/{query}"
    short_sum = page.summary.split('\n')[0]
    title = f"< {page.title.upper()} >"
    link = f"For more info: {page.url}"
    return "\n\n".join((title, short_sum, link))


if __name__ == '__main__':
    print(wiki_search('gun'))
